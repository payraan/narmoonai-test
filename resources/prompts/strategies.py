...
* ساختار کلی بازار:
   ...
   * تأثیر تایم‌فریم دوم بر رفتار بازار در تایم‌فریم پایین‌تر را توضیح بده.

۳. تحلیل تایم‌فریم سوم (دو تایم‌فریم بالاتر)
🔍 تحلیل تایم‌فریم Z:
* اسم و تیکر نمودار
* قیمت فعلی: رقم دقیق فعلی بازار در این تایم‌فریم.
* روند بلندمدت: بازار در بلندمدت چه وضعیتی دارد؟ صعودی، نزولی یا رنج؟ با یک جمله توضیح بنویس (مثلاً "روند بلندمدت همچنان صعودی است و سقف تاریخی جدید نزدیک است" یا "قیمت زیر مقاومت تاریخی مهم قرار دارد و روند کلی فعلاً نزولی است").
* سطوح تاریخی مهم: حمایت‌ها و مقاومت‌های تاریخی کلیدی را با رقم دقیق بنویس و اشاره کن که قیمت فعلی نسبت به این سطوح در کجا قرار دارد (مثلاً "قیمت در نزدیکی مقاومت مهم بلندمدت قرار دارد").
* ساختار کلان بازار:
   * توضیح بده آیا روند بلندمدت در حال تضعیف یا تقویت است (مثلاً با نشانه‌هایی مثل واگرایی یا نزدیکی به نواحی اشباع).
   * هر نشانه‌ای از پایان یا ادامه روند بلندمدت (مثلاً تشکیل الگوهای برگشتی بزرگ یا شکست ساختارهای تاریخی).
...
* ترکیب و مقایسه تحلیل سه تایم‌فریم:
   ...
   * اگر یکی از تایم‌فریم‌ها با بقیه متفاوت بود، این تضاد را توضیح بده و درباره ریسک بیشتر و وضعیت بلاتکلیف هشدار بده.
* سیگنال نهایی معاملاتی (فقط یکبار، در همین بخش):
   ...
   * حتماً شرط ورود را هم بنویس (مثلاً "در صورت شکست مقاومت کلیدی همراه با حجم بالا و کندل پوشای صعودی").
   * اگر روندها متضاد یا بازار غیرقابل پیش‌بینی بود، تحلیل را با هشدار مناسب به پایان برسان.
* هشدار:
   * اگر قیمت به محدوده پرریسک، ناحیه لیکوئیدیتی یا سقف/کف حساس نزدیک می‌شود، حتماً هشدار بده.
   * اگر دارایی موردنظر توکن پرریسک یا میم‌کوین بود، درباره ریسک پامپ/دامپ یا نوسان شدید بنویس.
* توصیه مدیریت ریسک:
