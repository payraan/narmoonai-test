...
async def dex_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "**به «نارموون دکس»، بخش تخصصی توکن‌های DEX خوش آمدید!** 📈\n\n"
        "در اینجا مرجع کاملی از اطلاعات مربوط به توکن‌های دکس در اختیار شما قرار میگیرد.\n\n"
        "با نارموون دکس برای یک بررسی هوشمندانه همراه باشید. 💎\n\n"
        "لطفاً یکی از گزینه‌ها را انتخاب کنید:",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode='Markdown'
    )
...
async def coin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "💰 **نارموون کوین**\n\n"
        "اینجا پایگاه جامع شما برای تحلیل و بررسی دنیای رمزارزهاست. از بیت‌کوین تا جدیدترین آلتکوین‌ها، تمام اطلاعات مورد نیازتان را در اختیار شما میگذارد\n\n"
        "با نارموون کوین به عمق بازار کریپتو سفر کنید. 📈\n\n"
        "لطفاً یکی از گزینه‌ها را انتخاب کنید:",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode='Markdown'
    )
...
async def trade_coach_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    message_text = (
        "🧠 **مربی هوش مصنوعی ترید نارموون** در خدمت شماست!\n\n"
        "از مدیریت سرمایه و کنترل ریسک تا روانشناسی بازار و استراتژی‌های حرفه‌ای، "
        "همه چیز رو باهم یاد می‌گیریم! 📈\n\n"
        "نمودارتون رو بفرستید یا سوالتون رو مطرح کنید، مربی شخصی‌تون آماده راهنماییه! 💡\n\n"
